...
class LineChart extends React.Component {
  ...
  displaysFrameOnTheLeft(gap) {
    if (gap > 0) {
      const calc = ((gap / 300) % 1).toFixed(2);
      return calc >= 0.8 || calc == 0.0;
    }
    return false;
  }
  ...
  drawSelected(index) {
    ...
    if (
      ...
      const displayLeft = this.displaysFrameOnTheLeft(dataObject.gap);
      let gap = 0;
      ...
      return (
        ...
        >
          ...
          <View
            style={StyleSheet.flatten([
              styles.selectedBox,
              displayLeft ? { marginLeft: gap - 105 } : {},
            ])}
          >
            {this.state.sortedData.map((series) => {
